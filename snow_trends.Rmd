...
snow_for_graphs %>% 
  filter( metric == "tot.snow"| metric == "max.snow.depth", site == "pinkham")%>%
   mutate(in_meters = value/1000,
         cm_per_decade = days_per_decade/10,
         names = ifelse(metric == "max.snow.depth", "Maximum snowpack depth", "Total snowfall"))%>%
  ggplot(aes(x = date, y = in_meters, color = cm_per_decade)) +
  geom_point(size = 1)+
  geom_line(size = .3)+
  geom_smooth(method = lm, size =.5)+
  theme_bw()+
  facet_wrap(~ names, scales = "free_y", nrow = 2)+
  labs( y = "Meters", x = "Year (1931-2018)", color = "Rate of loss (cm per decade)", title = "Pinkham Notch Snow Record")+
...
ggsave("snow.png", width = 5.25, height = 6.74, units = "in", dpi = 1000)
